...
\begin{document}
\thispagestyle{main}
\noindent \small First quarter U.S. economic growth slowed considerably from lower consumption spending, despite an improvement in exports. The unemployment rate is gradually nearing its long-run full-capacity level. Inflation is around two percent in both the core CPI and the full index, with no expected acceleration. Elevated consumer confidence and P/E ratios, with near long-term low equity market volatility, show an extraordinary level of optimism. The yield curve for government debt has flattened while the dollar remains strong.
...
\section{Economic Growth and Employment}
	...
	% GDP Growth by Component
		\indent \begin{tikzpicture}
			...
			\addplot[ybar stacked, bar width=2.6pt, color=blue!40!green, 
				...
				{data/gdp_comp.csv};			
			\addplot[thick, color=black] table [x=date, y=A191RL, col sep=comma] {data/gdp_comp.csv}
				node[pos=0.55,pin=80:{Real GDP Growth}] {};	
	...
	% Unemployment rate
		...
		\end{tikzpicture} \\
	% Footer: CPI 	
		\cf[Bureau of Economic Analysis: 
		\href{https://www.bea.gov/iTable/index_nipa.cfm}{NIPA Table 1.1.2.}]
